...
function generateDynamicUUID(key) {
    function getWeekOfYear() {
        ...
        const start = new Date(2007, 6, 7, updateTime, 0, 0); 
        const diff = adjustedNow - start;
        const oneWeek = 1000 * 60 * 60 * 24 * effectiveTime;
        return Math.ceil(diff / oneWeek);
    }

    const passwdTime = getWeekOfYear(); 
    const endTime = new Date(2007, 6, 7, updateTime, 0, 0); 
    ...
    const expirationDateUTC = new Date(endTime.getTime() - 8 * 60 * 60 * 1000); 
